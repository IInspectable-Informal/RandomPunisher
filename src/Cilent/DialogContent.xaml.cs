...
namespace RandomPunisher
{
    public sealed partial class DialogContent : UserControl
    {
        ...
        private void ShowTip(object sender, TappedRoutedEventArgs e)
        { ((sender as FrameworkElement).Tag as muxc.TeachingTip).IsOpen = true; }

        private void SetTipContent(object sender, RoutedEventArgs e)
        {
            Button s = sender as Button; muxc.TeachingTip Tip = s.Tag as muxc.TeachingTip; string str = "";
            foreach (string type in Tip == VidTypesShow ? CurApp.VidPicker.FileTypeFilter : CurApp.PicPicker.FileTypeFilter)
            { str += "*" + type + " "; } Tip.Subtitle = str + "\b"; s.Click -= SetTipContent;
        }

        private void AddEvent(object sender, RoutedEventArgs e)
        { Button s = sender as Button; s.Click += SetTipContent; s.Loaded -= AddEvent; }
        ...
        private void SetContent(object sender, RoutedEventArgs e)
        {
            switch (Content)
            {
                ...
                case "GetURL": ErrorReport.Visibility = GetURL.Visibility = Visibility.Visible ; break;
        ...
        private async void GetDrives(object sender = null, RoutedEventArgs e = null)
        {
            try
            {
                DrivesList.IsEnabled = false; AllDrivesList.Clear(); IDictionary<string, object> Prop;
                foreach (DriveInfo Drive in DriveInfo.GetDrives())
                {
                    try
                    {
                        Prop = await (await StorageFolder.GetFolderFromPathAsync(Drive.RootDirectory.ToString())).Properties.RetrievePropertiesAsync(new string[] { "System.FreeSpace", "System.Capacity" });
